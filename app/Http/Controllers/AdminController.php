...
class AdminController extends Controller
{
    ...
    public function index(Request $request) {
        ...
        $bus_today = Bus::where('status',1)->whereDate('created_at', '=', date('Y-m-d'))->get();
        $bus_month = Bus::where('status',1)->whereYear('created_at', '=', date('Y'))->whereMonth('created_at', '=', date('m'))->get();
        $bus_year = Bus::where('status',1)->whereYear('created_at', '=', date('Y'))->get();

        // Personnel
        $personnels = Personnel::where('status',1)->where('status',1)->get();
        $personnel_today = Personnel::where('status',1)->whereDate('created_at', '=', date('Y-m-d'))->get();
        $personnel_month = Personnel::where('status',1)->whereYear('created_at', '=', date('Y'))->whereMonth('created_at', '=', date('m'))->get();
        $personnel_year = Personnel::where('status',1)->whereYear('created_at', '=', date('Y'))->get();
        ...
        foreach($jan_ongoing as $val){
            if($val->personnel_schedule->schedule->company_id == $company_id){
                $cnt1++;
            }
        }
        ...
        foreach($bus_trip as $bt){
            if($bt->personnel_schedule->schedule->company_id == $company_id){
        ...
        return view('admin.dashboard',compact(
            ...
            'buses','bus_today', 'bus_month', 'bus_year',
            'personnels','personnel_today', 'personnel_month', 'personnel_year',
