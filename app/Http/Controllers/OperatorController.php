...
class OperatorController extends Controller
{
    ...
    public function index(Request $request) {
        ...
        foreach($jan_ongoing as $val){
            if($val->personnel_schedule->operator_id == $personnel_id){
                $cnt1++;
            }
        }
        ...
        foreach($bus_trip as $bt){
            if($bt->personnel_schedule->operator_id == $personnel_id){
        ...
        $announce = Reminder::where([['company_id', $com_id],['user_type',1]])->orWhere('user_type',4)->orWhere('user_type',6)->latest('created_at')->first();
