...
@section('scripts')
    ...
    <script>
            ...
            function initialize() {
                ...
                directionsService.route(request, function(response, status) {
                    if (status == google.maps.DirectionsStatus.OK) {
                        ...
                        if(@json($trip->inverse) == 0){
                            createMarker(endLocation.latlng, "Destination", @json($destination), "http://www.google.com/mapfiles/markerB.png");
                            createMarker(startLocation.latlng, "Origin ("+response.routes[0].legs[0].duration_in_traffic.text+") ("+response.routes[0].legs[0].distance.text+")", @json($origin), "http://www.google.com/mapfiles/marker_greenA.png");
                        }else{
                            createMarker(endLocation.latlng, "Destination", @json($origin), "http://www.google.com/mapfiles/markerB.png");
                            createMarker(startLocation.latlng, "Origin ("+response.routes[0].legs[0].duration_in_traffic.text+") ("+response.routes[0].legs[0].distance.text+")", @json($destination), "http://www.google.com/mapfiles/marker_greenA.png");
                        }
        ...
        function updatePosition(newLat, newLng){
            ...
            directionsDisplay.setMap(map);
            var start = new google.maps.LatLng(@json($orig_latitude), @json($orig_longitude));
            var new_start = new google.maps.LatLng(newLat, newLng);
            var end = new google.maps.LatLng(@json($dest_latitude), @json($dest_longitude));
            var request = {
                origin: new_start,
                destination: end,
            ...
            directionsService.route(request, function(response, status) {
                if (status == google.maps.DirectionsStatus.OK) {
                    Controller.Post('/api/trip/departure', { 'trip_id': @json($trip->id) }).done(function(res) {
                        ...
                        // createMarker(start, "Origin ("+response.routes[0].legs[0].duration_in_traffic.text+") ("+response.routes[0].legs[0].distance.text+")", startLocation.address, "http://www.google.com/mapfiles/marker_greenA.png");
                        
                        
                            // createMarker(end, "Destination", @json($destination), "http://www.google.com/mapfiles/markerB.png");
                            // createMarker(start, "Origin ("+response.routes[0].legs[0].duration_in_traffic.text+") ("+response.routes[0].legs[0].distance.text+")", @json($origin), "http://www.google.com/mapfiles/marker_greenA.png");
                        ...
                        if(@json($trip->inverse) == 0){
                            ...
                            createMarker(endLocation.latlng, "Destination", @json($origin), "http://www.google.com/mapfiles/markerB.png");
                            createMarker(start, "Origin ("+response.routes[0].legs[0].duration_in_traffic.text+") ("+response.routes[0].legs[0].distance.text+")", @json($destination), "http://www.google.com/mapfiles/marker_greenA.png");
                        }
